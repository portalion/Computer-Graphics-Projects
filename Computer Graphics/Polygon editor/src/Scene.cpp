...
void Scene::DisplayMenu()
{
    ...
    if (ImGui::GetIO().KeyCtrl && ImGui::GetIO().MouseClicked[ImGuiMouseButton_Left])
    {
        Circle* tmp = new Circle();
        tmp->x = ImGui::GetIO().MousePos.x;
        tmp->y = Scene::m_Height - ImGui::GetIO().MousePos.y;
        tmp->r = 100;
...
void Scene::Run()
{
    ...
    polygons[1]->AddPointAfterActive({ 900.f, 200.f });
